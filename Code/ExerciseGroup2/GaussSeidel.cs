...
namespace CSharpExerciseGroups_spiroskou.ExerciseGroup2
{
    class GaussSeidel
    {
        public static (double[] x, double rfinal) GaussSeidelSolver(double[,] A, double[] b, double accuracy, int maxIter, double[] y_init)
        {
            ...
            return (x, rfinal);  //converges faster and for wider variety of matrices A
        }


        public static void TestGaussSeidelSolver()
        {
            ...
            (x, rfinal) = GaussSeidelSolver(A, b, accuracy, maxIter, y_init);
        ...
        public static (double[] x, double rfinal) SORSolver(double[,] A, double[] b, double accuracy, int maxIter, double[] y_init)
        {
            double[] x = new double[A.GetLength(0)];
            double rfinal = 0;
            double w = 1.5;
            ...
            for (int iter = 1; iter < maxIter; iter++)
            {
                ...
                for (int i = 0; i < A.GetLength(0); i++)
                {
                    ...
                    y[i] = (1 - w) * y_init[i] + (b[i] - w * y[i]) / A[i, i];
        ...
        public static void TestSORSolver()
        {
            double[,] A = new double[4, 4] { { 15, 4, 1, 3 }, { 3, 24, 6, 2 }, { 1, 0, 44, 4 }, { 0, 1, 4, 19 } };
            double[] b = new double[4] { 1, 2, 3, 4 };
            double[] y_init = new double[4] { 0, 0, 0, 0 };
            ...
            (x, rfinal) = SORSolver(A, b, accuracy, maxIter, y_init);
